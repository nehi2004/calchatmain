...
//    const getAttachmentKind = (attachment: NoteAttachment) => {
...
//    const getAttachmentBadgeClasses = (kind: string) => {
//        switch (kind) {
//            case "image":
...
//    const getAttachmentLabel = (kind: string) => {
//        switch (kind) {
//            case "image":
//                return "Image"
//            case "pdf":
...
//    const AttachmentIcon = ({ attachment }: { attachment: NoteAttachment }) => {
...
//        if (kind === "pdf" || kind === "document" || kind === "presentation") {
...
//            if (!detailOpen || !selectedNote?.attachments?.length) return

//            const imageAttachments = selectedNote.attachments.filter(
//                (attachment) =>
//                    getAttachmentKind(attachment) === "image" && !imagePreviewUrls[attachment.id]
//            )
...
//                try {
//                    setLoadingPreviewIds((prev) => [...prev, attachment.id])

//                    const res = await fetch(
//                        `https://steadfast-warmth-production-64c8.up.railway.app/api/Notes/attachment/${attachment.id}/view`,
//                        {
//                            headers: {
//                                Authorization: `Bearer ${localStorage.getItem("token")}`,
//                            },
...
//                } finally {
//                    setLoadingPreviewIds((prev) => prev.filter((id) => id !== attachment.id))
//                }
//            }
//        }

//        loadImagePreviews()
//    }, [detailOpen, selectedNote, imagePreviewUrls])
...
//                                                getAttachmentBadgeClasses(kind)
//                                            )}
//                                        >
...
//                                                <span className="text-xs text-muted-foreground">
//                                                    {selectedNote.attachments.length} file
//                                                    {selectedNote.attachments.length > 1 ? "s" : ""}
//                                                </span>
//                                            )}
//                                        </div>
//                                    </div>
...
//                                                                    </div>
//                                                                ) : (
//                                                                    <div className="flex h-40 flex-col items-center justify-center gap-3 rounded-xl bg-muted text-center">
//                                                                        <AttachmentIcon attachment={attachment} />
//                                                                        <span
//                                                                            className={cn(
...
interface StoredNoteNotification {
    id: string
    userId: string
    type: string
    isRead: boolean
...
function parseStoredNotifications(): StoredNoteNotification[] {
    try {
...
function writeStoredNotifications(items: StoredNoteNotification[]) {
    localStorage.setItem("globalNotifications", JSON.stringify(items))
    window.dispatchEvent(new Event("new-notification"))
}

function upsertLocalNoteNotification(
    ...
    const filtered = existing.filter(
        (item) =>
            !(item.eventId === note.id && item.userId === userId && item.type.startsWith("note_"))
    )

    const message =
        type === "note_created"
            ? `${actorName} shared note "${note.title}" in ${note.category}${note.attachmentCount ? ` with ${note.attachmentCount} attachment${note.attachmentCount > 1 ? "s" : ""}` : ""}.`
            : type === "note_updated"
                ? `${actorName} updated note "${note.title}" in ${note.category}${note.attachmentCount ? ` with ${note.attachmentCount} attachment${note.attachmentCount > 1 ? "s" : ""}` : ""}.`
                : `${actorName} deleted note "${note.title}".`

    filtered.unshift({
        id: `${type}-${note.id}-${userId}`,
        userId,
        type,
        isRead: false,
...
async function sendNoteNotifications(
    type: "note_created" | "note_updated" | "note_deleted",
    payload: {
        id: string
        title: string
    ...
    await Promise.all(
        payload.userIds
            ...
            .map((userId) =>
                fetch("https://steadfast-warmth-production-64c8.up.railway.app/api/notifications", {
                    method: "POST",
                    ...
                    body: JSON.stringify({
                        ...
                        content:
                            type === "note_created"
                                ? `${actorName} shared note "${payload.title}" in ${payload.category}`
                                : type === "note_updated"
                                    ? `${actorName} updated note "${payload.title}" in ${payload.category}`
                                    : `${actorName} deleted note "${payload.title}"`,
                        type,
                        status: "info",
                        isRead: false,
                        title: payload.title,
        ...
        console.error("Note notification send error:", error)
    })
}
...
export function NotesView() {
    ...
    const currentUserName =
        typeof window !== "undefined" ? localStorage.getItem("name") || "HR" : "HR"
    ...
    async function handleSaveNote() {
        ...
        try {
            const previousNote = editMode ? notes.find((note) => note.id === editId) : null
            const formData = new FormData()
            formData.append("Title", newNote.title)
            formData.append("Content", newNote.content)
            formData.append("Category", newNote.category)
            ...
            try {
                ...
                savedId = parsed?.id || parsed?.noteId || savedId
            } catch {
                if (!savedId) savedId = crypto.randomUUID()
            }

            const attachmentCount = selectedFiles.length + (editMode ? existingAttachments.length : 0)
            const affectedUsers = editMode
                ? Array.from(new Set([...(previousNote?.userIds || []), ...selectedUsers]))
                : selectedUsers

            await sendNoteNotifications(
                editMode ? "note_updated" : "note_created",
                {
                    id: savedId,
                    title: newNote.title,
                    category: newNote.category,
            ...
            if (noteToDelete) {
                await sendNoteNotifications(
                    "note_deleted",
                    {
                        ...
                        userIds: noteToDelete.userIds || [],
                        attachmentCount: noteToDelete.attachments?.length || 0,
                    },
                    currentUserId || "",
                    currentUserName
                )
