...
export function NotesView() {
    ...
    useEffect(() => {
        if (role?.toLowerCase() === "hr") {
            fetch("http://localhost:5009/api/hr/employees", {
                headers: {
    ...
    async function handleSaveNote() {
        ...
        const url = editMode
            ? `http://localhost:5009/api/Notes/${editId}`
            : `http://localhost:5009/api/Notes/create`
    ...
    const fetchNotes = async () => {
        const res = await fetch("http://localhost:5009/api/Notes", {
            headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        })
    ...
    async function handleDeleteNote(id: string) {
        ...
        const res = await fetch(`http://localhost:5009/api/Notes/${id}`, {
            method: "DELETE",
            headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
